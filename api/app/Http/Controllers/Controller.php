...
{
    protected function jwt($user, $role) {
        $payload = [
            'iss' => "lumen-jwt", // Issuer of the token
            'sub' => $user->id."-".$user->nama_lengkap."-".$user->email."-".$role, // Subject of the token
    ...
    protected function jwtAdmin($user, $role) {
        $payload = [
            'iss' => "lumen-jwt", // Issuer of the token
            'sub' => $user->id."-".$user->username."-".$role, // Subject of the token
            'iat' => time(), // Time when JWT was issued.
            'exp' => time() + 60*60*8 // Expiration time
        ];
